...
export default function SellerProfile() {
	...
	const [name, setName] = useState(
		userStore.user?.first_name + " " + userStore.last_name,
	);
		...
		if (!userStore.user) {
			userStore.fetchUser(email).then(() => {
				...
				if (user) {
					...
					setAvatar(
						user.profile_pic ?? "/assets/images/profile_pic_placeholder.jpg",
					);
				}
			});
		}
	...
	const handleItemPress = (title: string) => {
		...
		if (title === "My Account") {
			setModalContent(<SellerAccount onClose={() => setModalVisible(false)} />);
		}
		if (title === "Store Profile") {
			setModalContent(
				<SellerBusiness onClose={() => setModalVisible(false)} />,
			);
		}
	};
	...
	const filteredProfileItems = profileItems.filter(
		(item: any) =>
			item.title === "Store Profile" || item.title === "My Account",
	);
		...
		<ProtectedRoute>
			<Container>
				...
				<View className="h-1/4 w-3/4  items-center justify-end mt-7">
					<Text className="text-2xl font-Poppins">Seller Profile</Text>
